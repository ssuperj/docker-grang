...
async function initPage() {
  ...
  const eventSource = new EventSource(
    `http://${currentHostname}:8100/sender/${sendUser.username}/receiver/${recvUser.username}`
  );
...
function initMessage(data, role) {
  ...
  spanDate.innerText = `${String(now.getFullYear()).substring(2, 4)} / ${String(now.getMonth()).padStart(
    2,
    "0"
  ...
  if (now.getHours() > 12) {
    spanTime.innerText = `오후 ${String(now.getHours() - 12).padStart(2, "0")} : ${String(now.getMinutes()).padStart(
      2,
      "0"
    )}`;
  } else {
    spanTime.innerText = `오전 ${String(now.getHours()).padStart(2, "0")} : ${String(now.getMinutes()).padStart(
      2,
      "0"
    )}`;
  }
...
async function addMyMessage() {
  ...
  fetch(`http://${currentHostname}:8100/chat`, {
    method: "post",
